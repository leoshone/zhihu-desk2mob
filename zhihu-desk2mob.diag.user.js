...
(function () {
  ...
  function setupModalBack() {
    ...
    // ===== DIAG：肉眼可见的调试面板（诊断版专用，发布版移除）=====
    var dbg = document.getElementById('zf-diag-panel');
    if (!dbg) {
      dbg = document.createElement('div');
      dbg.id = 'zf-diag-panel';
      dbg.style.cssText = 'position:fixed;right:4px;top:4px;z-index:2147483647;max-width:46vw;' +
        'background:rgba(0,0,0,.82);color:#9f9;font:11px/1.35 monospace;padding:6px 8px;' +
        'border-radius:6px;white-space:pre-wrap;max-height:60vh;overflow:auto;pointer-events:none;';
      (document.body || document.documentElement).appendChild(dbg);
    }
    window.__zfDiag = window.__zfDiag || { init: false, ticks: 0, lastFind: '', pushed: false, state: null, log: [] };
    window.__zfDiag.init = true;
    function zfDiagLog(m) {
      window.__zfDiag.log.unshift('[' + new Date().toLocaleTimeString() + '] ' + m);
      if (window.__zfDiag.log.length > 14) window.__zfDiag.log.pop();
      try {
        dbg.textContent = 'DIAG v0.7.4\ninit=1 ticks=' + window.__zfDiag.ticks +
    ...
    zfDiagLog('setupModalBack 初始化完成');
    ...
    function onModalOpen() {
      ...
      try {
        ...
        pushed = true;
        window.__zfDiag.pushed = true;
        window.__zfDiag.state = history.state;
        zfDiagLog('onModalOpen: 压入缓冲成功 state=' + JSON.stringify(history.state));
        log('弹层：压入缓冲历史');
      } catch (e) {
        window.__zfDiag.pushed = false;
        zfDiagLog('onModalOpen: pushState 抛错! ' + (e && e.message));
      }
    }
    ...
    function onModalGone() {
      ...
      try {
        ...
        if (history.state && history.state.zfModal) {
          silent = true;
          zfDiagLog('onModalGone: 清理缓冲 history.back()');
          history.back();
        }
    ...
    window.addEventListener('popstate', function () {
      if (silent) { silent = false; pushed = false; window.__zfDiag.pushed = false; zfDiagLog('popstate: silent 清理，忽略'); return; }
      // 常规判据没认出来的评论层，用宽松档再捞一次：
      // 宁可多关一个层，也不能让用户按了返回却被送离正文页
      var m = findOpenModalLoose();
      zfDiagLog('popstate: 用户返回, findOpenModalLoose=' + (m ? (m.className||m.tagName).toString().slice(0,20) : 'null'));
      if (!m) { pushed = false; window.__zfDiag.pushed = false; return; }
      normalizeLayer(m);
