"""真机：禁用滚动恢复 → reload 到顶部（评论区未加载）→ 点评论 → 是否弹窗

上一轮 reload 后 scrollY=33689（浏览器恢复滚动位置），评论区已渲染，
所以点评论一直是内联滚动定位。本轮用 history.scrollRestoration='manual' 强制回顶部。
"""
...
with sync_playwright() as p:
    ...
    # 禁用滚动恢复 + 回到顶部
    pg.evaluate("() => { try { history.scrollRestoration = 'manual'; } catch(e){} window.scrollTo(0,0); }")
    pg.wait_for_timeout(500)
    print("重新加载（已禁用滚动恢复）…")
    pg.goto(TARGET, wait_until="domcontentloaded", timeout=60000)
    pg.wait_for_timeout(4000)
    s0 = pg.evaluate(SNAP)
    print("  加载后: scrollY=%s | 评论区已渲染=%s | 弹层=%s" % (
        s0["y"], s0["cmtLoaded"], "在" if s0["modal"] else "无"))
    ...
    s1 = pg.evaluate(SNAP)
    print("  点后: 弹层=%s | 栈顶=%s | scrollY=%s | 评论区已渲染=%s" % (
        json.dumps(s1["modal"], ensure_ascii=False) if s1["modal"] else "无",
        json.dumps(s1["st"]), s1["y"], s1["cmtLoaded"]))
    ...
    if s1["modal"]:
        ...
        print()
        print("  仍未弹窗 —— 说明该入口在当前状态下就是内联展开。")
        print("  列出页面上所有「评论」入口及其位置，供确认：")
        cands = pg.evaluate("""() => {
            const out = [];
